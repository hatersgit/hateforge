    ...
    if (itr != _factions.end())
    {
        ...
        if (sScriptMgr->OnPlayerReputationChange(_player, factionEntry->ID, standing, incremental))
        {
            ...
            if (itr->second.Standing > 0 && !atLogin) {
                ...
                CharacterDatabase.Query("INSERT INTO character_accountwide_reputation (accountId, standing, factionId) VALUES ({}, {}, {}) ON DUPLICATE KEY UPDATE value = {}", accountId, _player->TeamIdForRace(_player->getRace()), factionEntry->ID, itr->second.Standing);
            }
