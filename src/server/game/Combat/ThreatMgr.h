...
public:
    ...
    bool isThreatListEmpty() { return _sortedThreatList.empty(); }
    // fastest of the three threat list getters - gets the threat list in "arbitrary" order
    Acore::IteratorPair<threat_list_heap::const_iterator> GetUnsortedThreatList() const { return { _sortedThreatList.begin(), _sortedThreatList.end() }; }
    ...
    Acore::IteratorPair<threat_list_heap::ordered_iterator> GetSortedThreatList() const { return { _sortedThreatList.ordered_begin(), _sortedThreatList.ordered_end() }; }
...
public:
    ThreatMgr(ThreatMgr const&) = delete;
    ThreatMgr& operator=(ThreatMgr const&) = delete;
...
private:
    ThreatReference(ThreatMgr* mgr, Unit* victim, float amount) : _owner(mgr->_owner), _mgr(mgr), _victim(victim), _baseAmount(amount), _tempModifier(0), _online(SelectOnlineState()), _taunted(TAUNT_STATE_NONE) { }
    ...
    OnlineState _online;
    TauntState _taunted;
    ThreatMgr::threat_list_heap::handle_type _handle;
public:
