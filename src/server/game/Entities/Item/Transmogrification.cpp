...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::GetSlotName");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::GetItemIcon");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::UpdateItem");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::GetSpecialPrice");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::IsBowOrGunOrCrossbow");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::IsMeleeWeapon");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::IsAllowed");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::IsNotAllowed");
...
{
    LOG_DEBUG("custom.transmog", "Transmogrification::Transmogrify");
...
TransmogResult Transmogrification::CannotEquip(Player* player, ItemTemplate const* proto)
{
    if (!IgnoreReqFaction)
    {
        if ((proto->Flags2 & ITEM_FLAG2_FACTION_HORDE) && player->GetTeamId() != HORDE)
            return TransmogResult_InvalidFaction;

        if ((proto->Flags2 & ITEM_FLAG2_FACTION_ALLIANCE) && player->GetTeamId() != ALLIANCE)
            return TransmogResult_InvalidFaction;
    }
...
void Transmogrification::AddToCollectionEnchant(Player* player, uint32 enchant_id)
{
    ...
    if (uint32 enchant = AddEnchantVisualToCollection(player, enchant_id))
    {
        ...
        if (enchantEntry)
        {
            auto it = enchant_visual_to_name.find(enchantEntry->aura_id);
