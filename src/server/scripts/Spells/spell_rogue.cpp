...
enum RogueSpells
{
    ...
    SPELL_ROGUE_KILLING_SPREE                   = 100068,
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        while (!_targets.empty())
        {
            ...
            if (Unit* target = ObjectAccessor::GetUnit(*GetTarget(), guid))
            {
                ...
                if (GetTarget()->ToPlayer()->HasAura(110221))
                    GetTarget()->CastSpell(target, 100074, TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_DONT_REPORT_CAST_ERROR));
...
class spell_rog_killing_spree_SpellScript : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        {
            if (spell_rog_killing_spree_AuraScript* script = dynamic_cast<spell_rog_killing_spree_AuraScript*>(aura->GetScriptByName("spell_rog_killing_spree_AuraScript")))
...
void AddSC_rogue_spell_scripts()
{
    ...
    RegisterSpellScript(spell_rog_killing_spree_SpellScript);
    RegisterSpellScript(spell_rog_killing_spree_AuraScript);
