...
std::string _SpellScript::EffectNameCheck::ToString()
{
    switch (effName)
    {
        ...
        default:
            char num[10];
            snprintf(num, sizeof(num), "{}", effName);
            return num;
    }
}
...
std::string _SpellScript::EffectAuraNameCheck::ToString()
{
    switch (effAurName)
    {
        ...
        default:
            char num[10];
            snprintf(num, sizeof(num), "{}", effAurName);
            return num;
    }
}
...
bool AuraScript::SpellIsCrit(ProcEventInfo& eventInfo)
{
    ...
    return (damageInfo && (damageInfo->GetHitInfo() & HITINFO_CRITICALHIT)) || eventInfo.GetHitMask() & PROC_HIT_CRITICAL;
}
