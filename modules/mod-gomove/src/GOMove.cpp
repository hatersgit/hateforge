...
GameObject * GOMove::SpawnGameObject(Player* player, float x, float y, float z, float o, uint32 p, uint32 entry)
{
    ...
    if (!object->Create(guidLow, objectInfo->entry, map, player->GetPhaseMaskForSpawn(), pos.GetPositionX(),
        pos.GetPositionY(), pos.GetPositionZ(), pos.GetOrientation(), G3D::Quat(), 255, GO_STATE_READY))
    {
        delete object;
        return nullptr;
    }
...
GameObject * GOMove::MoveGameObject(Player* player, float x, float y, float z, float o, uint32 p, ObjectGuid::LowType lowguid)
{
    ...
    object = new GameObject();
    if (!object->LoadFromDB(lowguid, map))
    {
        delete object;
        SendRemove(player, lowguid);
        return nullptr;
