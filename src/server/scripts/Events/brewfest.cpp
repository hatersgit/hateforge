...
struct npc_dark_iron_attack_generator : public ScriptedAI
{
    ...
    void FinishEventDueToLoss()
    {
        ...
        {
            char amount[500];
            snprintf(amount, sizeof(amount), "We did it boys! Now back to the Grim Guzzler and we'll drink to the {} that were injured!", guzzlerCounter);
    ...
    void FinishAttackDueToWin()
    {
        ...
        {
            char amount[500];
            snprintf(amount, sizeof(amount), "RETREAT!! We've already lost {} and we can't afford to lose any more!!", guzzlerCounter);
            herald->Yell(amount, LANG_UNIVERSAL);
        }
