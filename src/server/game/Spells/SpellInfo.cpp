...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/) const
{
    ...
    if (caster)
    {
                ...
                !basePointsPerLevel && _spellInfo->HasAttribute(SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL))
        {
            ...
            if (canEffectScale)
            {
                if (Creature const* cre = caster->ToCreature()) {
                    if (CreatureTemplate const* cInfo = caster->ToCreature()->GetCreatureTemplate()) {
...
{ {
    ...
    {EFFECT_IMPLICIT_TARGET_NONE    , TARGET_OBJECT_TYPE_DEST}, // 165 SPELL_EFFECT_CREATE_AREATRIGGER
} };
...
{
    return !(HasAttribute(SPELL_ATTR1_NO_THREAT) || HasAttribute(SPELL_ATTR3_SUPRESS_TARGET_PROCS));
}
