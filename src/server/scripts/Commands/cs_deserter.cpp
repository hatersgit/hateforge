...
public:
    ...
    */
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        {
            { "add",        HandleDeserterInstanceAdd,       SEC_GAMEMASTER2_F, Console::Yes },
            { "remove all", HandleDeserterInstanceRemoveAll, SEC_GAMEMASTER2_F, Console::Yes },
            { "remove",     HandleDeserterInstanceRemove,    SEC_GAMEMASTER2_F, Console::Yes }
        };
        static ChatCommandTable deserterBGCommandTable =
        {
            { "add",        HandleDeserterBGAdd,       SEC_GAMEMASTER2_F, Console::Yes },
            { "remove all", HandleDeserterBGRemoveAll, SEC_GAMEMASTER2_F, Console::Yes },
            { "remove",     HandleDeserterBGRemove,    SEC_GAMEMASTER2_F, Console::Yes }
        };
    ...
    */
    static bool HandleDeserterRemove(ChatHandler* handler, Optional<PlayerIdentifier> player, bool isInstance)
    {
        ...
        if (duration < 0)
        {
            handler->SendErrorMessage("Permanent {} Deserter has been removed from player {} (GUID {}).", isInstance ? "Instance" : "Battleground", handler->playerLink(player->GetName()), player->GetGUID().GetCounter());
            return true;
        }

        handler->PSendSysMessage("{} of {} Deserter has been removed from player {} (GUID {}).", secsToTimeString(duration / IN_MILLISECONDS), isInstance ? "Instance" : "Battleground", handler->playerLink(player->GetName()), player->GetGUID().GetCounter());
        return true;
    }
